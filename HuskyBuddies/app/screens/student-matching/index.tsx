...
export default function MatchingPage() {
  ...
  const findMatches = (criteria) => {
    setFilterCriteria(criteria);
    let filteredMatches = [];
    switch (criteria) {
      case 'classes':
        filteredMatches = studentProfiles.filter(student => 
  ...
  const renderStudentCard = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.infoText}>Interests: {item.interests.join(', ')}</Text>
  ...
  const renderHeader = () => (
    <View>
      ...
      <View style={styles.profileSection}>
        ...
        <View style={styles.profileInfo}>
          ...
          <View style={styles.profileText}>
            ...
            <Text>Interests: {studentProfiles[0].interests.join(', ')}</Text>
      ...
      <View style={styles.filterButtonsContainer}>
        <TouchableOpacity
          style={[styles.filterButton, filterCriteria === 'classes' && styles.activeFilterButton]}
          onPress={() => findMatches('classes')}
        >
          <Text style={styles.filterButtonText}>Find by Classes</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.filterButton, filterCriteria === 'interests' && styles.activeFilterButton]}
          onPress={() => findMatches('interests')}
        >
          <Text style={styles.filterButtonText}>Find by Interests</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.filterButton, filterCriteria === 'location' && styles.activeFilterButton]}
          onPress={() => findMatches('location')}
        >
          <Text style={styles.filterButtonText}>Find by Location</Text>
        </TouchableOpacity>
      </View>
